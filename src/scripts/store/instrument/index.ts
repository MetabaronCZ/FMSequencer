...
type SetInstrumentAlgorithmAction = PayloadAction<
  TrackActionPayload<AlgorithmID>
>;
...
export type InstrumentActions =
  | LoadIntrumentAction
  | ResetIntrumentAction
  | SetIntrumentNameAction
...
const resetInstrument: ProjectReducer<ResetIntrumentAction> = (
  state,
  action
) => {
...
const setInstrumentName: ProjectReducer<SetIntrumentNameAction> = (
  state,
  action
) => {
  const { track, data } = action.payload;
  state.tracks[track].instrument.name = data.substring(
    0,
    INSTRUMENT_NAME_LENGTH
  );
};

const setInstrumentAlgorithm: ProjectReducer<SetInstrumentAlgorithmAction> = (
  state,
  action
) => {
  const { track, data } = action.payload;
  ...
  if (!isAlgorithm(value)) {
    throw new Error(
      `Could not set algorithm: Invalid value "${action.payload}"!`
    );
  }
...
const setInstrumentLevel: ProjectReducer<SetInstrumentLevelAction> = (
  state,
  action
) => {
  const { track, data } = action.payload;
...
const setInstrumentPan: ProjectReducer<SetInstrumentPanAction> = (
  state,
  action
) => {
  const { track, data } = action.payload;
