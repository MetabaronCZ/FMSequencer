...
const removeSongSequence: ProjectReducer<RemoveSongSequenceAction> = (
  state,
  action
) => {
...
const moveSongSequence: ProjectReducer<MoveSongSequenceRepeatAction> = (
  state,
  action
) => {
...
const setSongSequence: ProjectReducer<SetSongSequenceAction> = (
  state,
  action
) => {
  const { slot, data } = action.payload;
  ...
  if (!seq) {
    throw new Error(
      `Could not update sequence: Invalid sequence slot "${slot}"!`
    );
  }
  seq.sequence = limitNumber(data, 0, SEQUENCE_COUNT - 1);
};

const setSongSequenceRepeat: ProjectReducer<SetSongSequenceRepeatAction> = (
  state,
  action
) => {
  const { slot, data } = action.payload;
