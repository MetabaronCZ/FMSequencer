...
import { StepFXID, StepFXType, STEP_FX_VALUE_MAX, STEP_FX_VALUE_MIN } from 'modules/project/config';
...
type SetTrackPatternStepVelocityAction = PayloadAction<StepActionPayload<number>>;
type SetTrackPatternStepFXTypeAction = PayloadAction<StepFXActionPayload<StepFXType>>;
type SetTrackPatternStepFXValueAction = PayloadAction<StepFXActionPayload<number>>;
...
const setTrackPatternStepPitch: ProjectReducer<SetTrackPatternStepPitchAction> = (state, action) => {
...
const deleteTrackPatternStepPitch: ProjectReducer<DeleteTrackPatternStepPitchAction> = (state, action) => {
...
const setTrackPatternStepVelocity: ProjectReducer<SetTrackPatternStepVelocityAction> = (state, action) => {
    const { track, pattern, step, data } = action.payload;
    const { steps } = state.tracks[track].patterns[pattern];
    const stepData = steps.find(({ start }) => step === start);
...
const setTrackPatternStepFXType: ProjectReducer<SetTrackPatternStepFXTypeAction> = (state, action) => {
    ...
    const fxData = stepData.fx[fx] ?? createStepFXData() as WritableDraft<StepFXData>;
    fxData.type = data;
    stepData.fx[fx] = fxData;
};

const setTrackPatternStepFXValue: ProjectReducer<SetTrackPatternStepFXValueAction> = (state, action) => {
    const { track, pattern, step, fx, data } = action.payload;
    const { steps } = state.tracks[track].patterns[pattern];
    let stepData = steps.find(({ start }) => step === start);
...
const deleteTrackPatternStepFX: ProjectReducer<DeleteTrackPatternStepFXAction> = (state, action) => {
