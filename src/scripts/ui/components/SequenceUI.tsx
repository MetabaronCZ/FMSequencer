...
export const SequenceUI: React.FC = () => {
    ...
    const { setSequenceTrackPattern, setSequenceLength, clearSequence } = projectSlice.actions;

    const { tracks: projectTracks, sequences } = useAppSelector((state) => state.project);
    const { sequence, track, mutedTracks, soloedTrack } = useAppSelector((state) => state.session);
